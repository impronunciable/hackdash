...
import {maxQueryLimit=50 as maxLimit} from 'config';
...
export const createDashboard = async (req, res, next) => {
  ...
  try {
    const savedDashboard = await newDashboard.save().exec();
...
const setDashboards = async (req, res, next) => {
  const limit = req.limit || maxLimit;
  try {
    const dashboards = awaitDashboard.find(req.search_query || {})
      .select('-__v')
